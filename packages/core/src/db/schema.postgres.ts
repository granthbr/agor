...
export const promptTemplates = pgTable(
  'prompt_templates',
  {
    template_id: varchar('template_id', { length: 36 }).primaryKey(),
    board_id: varchar('board_id', { length: 36 }).references(() => boards.board_id, {
      onDelete: 'set null',
    }),
    created_by: varchar('created_by', { length: 36 }).notNull().default('anonymous'),
    ...
    parent_id: varchar('parent_id', { length: 36 }), // Self-reference for forks
    is_latest: t.bool('is_latest').notNull().default(true),
    usage_count: integer('usage_count').notNull().default(0),
    avg_rating: integer('avg_rating').notNull().default(0),
    created_at: t.timestamp('created_at').notNull(),
...
export const promptRatings = pgTable(
  'prompt_ratings',
  {
    rating_id: varchar('rating_id', { length: 36 }).primaryKey(),
    template_id: varchar('template_id', { length: 36 })
      .notNull()
      .references(() => promptTemplates.template_id, { onDelete: 'cascade' }),
    session_id: varchar('session_id', { length: 36 }).references(() => sessions.session_id, {
    ...
    rated_by: varchar('rated_by', { length: 36 }).notNull().default('anonymous'),
